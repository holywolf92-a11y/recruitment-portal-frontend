...
import { Search, Plus, Building2, Mail, Phone, MapPin, Briefcase, Eye, Download } from 'lucide-react';

export function EmployerManagement() {
  const [searchTerm, setSearchTerm] = useState('');
  const [generatingCV, setGeneratingCV] = useState<string | null>(null);

  const handleGenerateEmployerCV = async (employerId: string, companyName: string) => {
    setGeneratingCV(employerId);
    try {
...
Email: falishamanpower4035@gmail.com
      ...
      element.setAttribute('download', `EmployerCV_${companyName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.txt`);
        ...
        {filteredEmployers.map((employer) => (
          <div key={employer.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="border-t border-gray-200 p-4 flex gap-2">
              ...
              <button
                onClick={() => handleGenerateEmployerCV(employer.id, employer.companyName)}
                disabled={generatingCV === employer.id}
                className="flex-1 px-4 py-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                title="Generate safe CV for this employer"
              >
                {generatingCV === employer.id ? (
                  <>
